...
def build_custom_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard_layout = [
        [
            InlineKeyboardButton("要聞", callback_data="要聞"),
            InlineKeyboardButton("產業", callback_data="產業"),
        ],
        [
            InlineKeyboardButton("證券", callback_data="證券"),
            InlineKeyboardButton("國際", callback_data="國際"),
        ],
        [
            InlineKeyboardButton("金融", callback_data="金融"),
            InlineKeyboardButton("期貨", callback_data="期貨"),
        ],
        [
            InlineKeyboardButton("理財", callback_data="理財"),
            InlineKeyboardButton("房市", callback_data="房市"),
        ],
        [
            InlineKeyboardButton("專欄", callback_data="專欄"),
            InlineKeyboardButton("專題", callback_data="專題"),
        ],
        [
            InlineKeyboardButton("商情", callback_data="商情"),
            InlineKeyboardButton("兩岸", callback_data="兩岸"),
        ],
...
def run_news_pipeline(category: str) -> list:
    ...
    with open(
        f"./cate_news/cate{source_dict[category]}_news.json", "r", encoding="utf-8"
    ) as f:
        news = json.load(f)
...
async def news_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("回上一頁", callback_data="back_to_2"),
            InlineKeyboardButton("結束", callback_data="end"),
        ],  # 回到 Page 2
    ]
...
async def full_article(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [
            InlineKeyboardButton("回上一頁", callback_data="back_to_3"),
            InlineKeyboardButton("結束", callback_data="end"),
        ],  # 回到 Page 3
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    await query.edit_message_text(
        text=f"【全文】:\n\n {specific_content}", reply_markup=reply_markup
    )
...
async def back_to_2(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        f"已返回【{category}】類新聞列表：", reply_markup=keyboard_markup
    )
...
async def end(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(text="下次見! 感謝使用本服務。")
...
def main():
    ...
    application.add_handler(
        CallbackQueryHandler(handle_button_press, pattern="^[\u4e00-\u9fff]+$")
    )
    ...
    # 3. 查看全文 (進入 Page 4)
    application.add_handler(
        CallbackQueryHandler(full_article, pattern="^full_article$")
    )

    # 4. 回退邏輯 (使用精確的 callback_data 命名)
    application.add_handler(
        CallbackQueryHandler(back_to_3, pattern="^back_to_3$")
    )  # P4 -> P3
    application.add_handler(
        CallbackQueryHandler(back_to_2, pattern="^back_to_2$")
    )  # P3 -> P2
    application.add_handler(
        CallbackQueryHandler(back_to_1, pattern="^back_to_1$")
    )  # P2 -> P1
...
if __name__ == "__main__":
    print("Data set already! Bot is starting...")
    main()
